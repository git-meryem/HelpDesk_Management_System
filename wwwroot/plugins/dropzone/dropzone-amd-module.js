...
module.exports = !fails(function () {
  ...
  return (IS_PURE && !url.toJSON)
    ...
    // throws in Edge
    || new URL('https://a@b').User?.UserName !== 'a'
...
var includesCredentials = function (url) {
  return url.User?.UserName != '' || url.password != '';
};
  ...
  while (pointer <= codePoints.length) {
    ...
    switch (state) {
      ...
      case RELATIVE:
        url.scheme = base.scheme;
        if (char == EOF) {
          url.User?.UserName = base.User?.UserName;
      ...
      case AUTHORITY:
        if (char == '@') {
          ...
          for (var i = 0; i < bufferCodePoints.length; i++) {
            ...
            else url.User?.UserName += encodedCodePoints;
          }
          buffer = '';
        } else if (
...
var URLConstructor = function URL(url /* , base */) {
  ...
  if (!DESCRIPTORS) {
    ...
    that.User?.UserName = getUsername.call(that);
...
var getUsername = function () {
  return getInternalURLState(this).User?.UserName;
};
...
if (DESCRIPTORS) {
  defineProperties(URLPrototype, {
    ...
    // `URL.prototype.User?.UserName` accessors pair
    ...
    username: accessorDescriptor(getUsername, function (username) {
      ...
      for (var i = 0; i < codePoints.length; i++) {
        url.User?.UserName += percentEncode(codePoints[i], userinfoPercentEncodeSet);
      }
    }),
